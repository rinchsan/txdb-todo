...
var UserRels = struct {
	AuthorUserTodos string
	Todos           string
}{
	AuthorUserTodos: "AuthorUserTodos",
	Todos:           "Todos",
}

// userR is where relationships are stored.
type userR struct {
	AuthorUserTodos TodoSlice `boil:"AuthorUserTodos" json:"AuthorUserTodos" toml:"AuthorUserTodos" yaml:"AuthorUserTodos"`
...
var (
	userAllColumns            = []string{"id", "username", "created_at", "updated_at"}
...
// AuthorUserTodos retrieves all the todo's Todos with an executor via author_user_id column.
func (o *User) AuthorUserTodos(mods ...qm.QueryMod) todoQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`todos`.`author_user_id`=?", o.ID),
	)
...
func (o *User) Todos(mods ...qm.QueryMod) todoQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("`todos_users` on `todos`.`id` = `todos_users`.`todo_id`"),
		qm.Where("`todos_users`.`user_id`=?", o.ID),
	)
...
// LoadAuthorUserTodos allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadAuthorUserTodos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`todos`),
		qm.WhereIn(`todos.author_user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.AuthorUserTodos = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.AuthorUserID {
				local.R.AuthorUserTodos = append(local.R.AuthorUserTodos, foreign)
				if foreign.R == nil {
					foreign.R = &todoR{}
				}
...
func (userL) LoadTodos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.Select("`todos`.*, `a`.`user_id`"),
		qm.From("`todos`"),
		qm.InnerJoin("`todos_users` as `a` on `todos`.`id` = `a`.`todo_id`"),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Title, &one.Detail, &one.AuthorUserID, &one.DueDate, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
			return errors.Wrap(err, "failed to scan eager loaded results for todos")
		}
		if err = results.Err(); err != nil {
			return errors.Wrap(err, "failed to plebian-bind eager loaded slice todos")
		}
...
// AddAuthorUserTodos adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.AuthorUserTodos.
// Sets related.R.AuthorUser appropriately.
func (o *User) AddAuthorUserTodos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Todo) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.AuthorUserID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			AuthorUserTodos: related,
		}
	} else {
		o.R.AuthorUserTodos = append(o.R.AuthorUserTodos, related...)
	}
...
func (o *User) AddTodos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Todo) error {
	...
	for _, rel := range related {
		query := "insert into `todos_users` (`user_id`, `todo_id`) values (?, ?)"
		values := []interface{}{o.ID, rel.ID}
...
func (o *User) SetTodos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Todo) error {
	...
	removeTodosFromUsersSlice(o, related)
	if o.R != nil {
		o.R.Todos = nil
	}
...
func (o *User) RemoveTodos(ctx context.Context, exec boil.ContextExecutor, related ...*Todo) error {
	var err error
	query := fmt.Sprintf(
		"delete from `todos_users` where `user_id` = ? and `todo_id` in (%s)",
...
func removeTodosFromUsersSlice(o *User, related []*Todo) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
func (o *User) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "entity: unable to populate default values for users")
	}
...
func (o *User) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "entity: unable to upsert for users")
	}
	...
	if err != nil {
		return errors.Wrap(err, "entity: unable to retrieve unique values for users")
	}
